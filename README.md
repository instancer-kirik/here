...
**🌐 [View Interactive Landing Page](https://findry.com/projects/here-universal-package-manager/landing)**
...
| `recover [service]` | Recover services from backup | `here recover --all` |
| `export [file]` | Export system profile for migration | `here export my-setup.json` |
| `import <file>` | Import and restore system profile | `here import my-setup.json` |
| `backup <source>` | Smart backup for system migration | `here backup ~` |
...
**here** includes powerful tools for system recovery and migration, similar to configuration management tools like Ansible but focused on restoring your development environment.

### Recovery Features

- **🐳 Docker**: Full Docker installation with authentication, daemon config, and CLI plugins
- **🦭 Podman**: Rootless containers with Docker compatibility layer
- **🐘 PostgreSQL/PostGIS**: Complete database cluster restoration with version handling
- **📁 Configuration**: Dotfiles, SSH keys, and application settings
- **🔧 Interactive**: Choose exactly which services to recover
...
# Recover specific services
here recover --docker --postgresql
here recover postgresql
here recover docker

# Export current system for migration
here export --include-config my-system.json

# Import system on new machine
here import --interactive my-system.json
here import --install-all my-system.json

# Smart backup before migration  
here backup ~ -d /mnt/backup/home
```

### Recovery Process

**here** automatically detects your backup location and guides you through recovery:

```bash
$ here recover --all
🔄 Recovery System
==================
✅ Found backup at: /run/media/bon/MainStorage/MAIN_SWAP/home-backup
🔍 Detected Arch Linux with yay

📋 Available recovery options:
   1. Docker only
   2. Podman only  
   3. PostgreSQL/PostGIS only
   4. Docker + PostgreSQL/PostGIS
   5. Podman + PostgreSQL/PostGIS
   6. All services (Docker + Podman + PostgreSQL/PostGIS)
   7. Custom selection

Choose recovery option [1-7]: 6

🎯 Recovery Plan:
   Docker: ✅ Yes
   Podman: ✅ Yes  
   PostgreSQL/PostGIS: ✅ Yes
...
📦 Installing Podman and related tools...
✅ Podman recovery completed!

🐘 Starting PostgreSQL recovery...
📊 Backup PostgreSQL version: 16
📦 Installing PostgreSQL and PostGIS...
📂 Restoring PostgreSQL data from backup...
✅ PostgreSQL recovery completed!
```
...
- Daemon configuration (garbage collection, storage settings)
- CLI plugins and contexts
- Docker Desktop configuration (if available)

#### Podman Recovery  
- Podman with Docker compatibility layer
- Container runtime configuration (crun, netavark)
- User namespace setup for rootless containers
- Optional existing container/image restoration
- Docker command aliases for seamless transition

#### PostgreSQL/PostGIS Recovery
- Complete PostgreSQL database cluster
- All existing databases and user data
- PostGIS extensions and spatial data support
- Configuration files (postgresql.conf, pg_hba.conf)
- Proper version compatibility handling

### Migration Workflow

**Complete system migration in 3 steps:**

1. **Export on old system:**
   ```bash
   here export --include-config full-system.json
   here backup ~ -d /mnt/external/backup
   ```

2. **Transfer to new system:**
   ```bash
   # Copy backup drive and profile to new machine
   scp full-system.json user@newmachine:~/
   ```

3. **Restore on new system:**
   ```bash
   here import --interactive full-system.json
   here recover --all
   ```
...
The recovery system is similar to Ansible playbooks but focused on development environments:

```bash
# Declarative: "I want these services restored"
here recover --docker --postgresql

# Idempotent: Safe to run multiple times  
here recover postgresql  # Won't break existing setup

# System-aware: Automatically detects package managers
# Arch: Uses pacman/yay
# Ubuntu: Uses apt + snap/flatpak fallback
# Fedora: Uses dnf + flatpak fallback
```

#### Backup Integration
Recovery works with the built-in backup system:

```bash
# Smart backup before migration
here backup ~ -d /external/backup --exclude-cache --include-hidden

# Automatic backup detection
here recover --all  # Automatically finds backups at common locations
```
